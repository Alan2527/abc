...
//ENVIAR DATOS DEL FORMULARIO A LA BASE DE DATOS:
    ...
    if(!empty($_POST['fecha'])) {
        ...
        if(!empty($_POST['id'])) {
            $id_maximo = $_POST['id'];
            mysqli_query($conexion, "UPDATE comusgbatos SET titulo='$titulo' texto='$texto' fecha='$fecha' WHERE id='$id_maximo'");
            echo 'SE HA ACTUALIZADO LA INFORMACIÓN CON EXITO<br>';
        }else {
            $sql=mysqli_query($conexion, "SELECT id FROM comusgbatos WHERE titulo='$titulo' texto='$texto' fecha='$fecha'");
            if($row=mysqli_fetch_array($sql)) {
                echo 'NO SE PERMITEN DATOS DUPLICADOS EN LA BASE DE DATOS<br><br>';
            }else{
                mysqli_query($conexion, "INSERT INTO comusgbatos (titulo, texto, fecha) VALUES ('$titulo', '$texto', '$fecha')");
                $ss=mysqli_query($conexion, "SELECT MAX(id) as id_maximo FROM comusgbatos");
                if($rr=mysqli_fetch_array($ss)) {
                    $id_maximo=$rr['id_maximo'];
                }
                echo "<script>
                alert('Carga exitosa!');
                window.location = 'cargas-sgbatos.php';
                </script>";
            }
        }
        ...
        $urlnueva = "imagen-sgbatos/foto_".$id_maximo.".jpg";
