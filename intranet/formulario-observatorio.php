...
//ENVIAR DATOS DEL FORMULARIO A LA BASE DE DATOS:
    ...
    if(!empty($_POST['fecha'])) {
        ...
        if(!empty($_POST['id'])) {
            $id_maximo = $_POST['id'];
            mysqli_query($conexion, "UPDATE comuident SET titulo='$titulo' texto='$texto' fecha='$fecha' WHERE id='$id_maximo'");
            echo 'SE HA ACTUALIZADO LA INFORMACIÓN CON EXITO<br>';
        }else {
            $sql=mysqli_query($conexion, "SELECT id FROM comuident WHERE titulo='$titulo' texto='$texto' fecha='$fecha'");
            if($row=mysqli_fetch_array($sql)) {
                echo 'NO SE PERMITEN DATOS DUPLICADOS EN LA BASE DE DATOS<br><br>';
            }else{
                mysqli_query($conexion, "INSERT INTO comuident (titulo, texto, fecha) VALUES ('$titulo', '$texto', '$fecha')");
                $ss=mysqli_query($conexion, "SELECT MAX(id) as id_maximo FROM comuident");
                if($rr=mysqli_fetch_array($ss)) {
                    $id_maximo=$rr['id_maximo'];
                }
                "<script>
                alert('Carga exitosa!');
                window.location = 'cargas-observatorio.php';
                </script>";
            }
        }
        ...
        $urlnueva = "imagen-obs/foto_".$id_maximo.".jpg";
